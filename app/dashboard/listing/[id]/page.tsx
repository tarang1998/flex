...
export default function ListingReportPage() {
  ...
  const [listingData, setListingData] = useState<ListingWithReviews | null>(null);
  ...
  const fetchListing = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/dashboard/listing/${listingId}`);
      ...
      if (data.success) {
        ...
        const uniqueChannels = Array.from(
          new Set(data.data.reviews.map((r: Review) => r.channel).filter(Boolean))
        ) as string[];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-lg">Loading review report...</p>
        </div>
      </div>
    );
  }
                    ...
                    <div key={channel} className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <span className="text-xs text-gray-500">{channelCount} reviews</span>
                          <span className={`text-sm font-bold ${
                            channelAvgRating >= 4.5 ? 'text-green-600' :
                            channelAvgRating >= 3.5 ? 'text-yellow-600' :
                            'text-red-600'
                          }`}>
                            {channelAvgRating.toFixed(1)}★
                          </span>
                        </div>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full ${
                            channelAvgRating >= 4.5 ? 'bg-green-500' :
                            channelAvgRating >= 3.5 ? 'bg-yellow-500' :
                            'bg-red-500'
                          }`}
                  ...
                  .map(([category, rating]) => (
                    <div key={category} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-bold text-gray-900 w-12 text-right">{rating.toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-lg">
                ...
                <div>
                  <div className="font-semibold text-gray-900">
                    {trend === 'up' ? 'Ratings Improving' :
                     trend === 'down' ? 'Ratings Declining' :
                     'Ratings Stable'}
                  </div>
              ...
              {lowRatingCount > 0 && (
                <div className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                  ...
                  <div>
                    <div className="font-semibold text-orange-900">{lowRatingCount} Low Ratings</div>
                    <div className="text-sm text-orange-700">Reviews below 3.5 stars need attention</div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  {categoryIssues.slice(0, 3).map(issue => (
                    <div key={issue.category} className="flex items-center justify-between text-sm p-2 bg-red-50 rounded">
                      <span className="capitalize text-red-900">{issue.category}</span>
                      <span className="font-semibold text-red-700">{issue.avgRating.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              )}
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            {review.rating && (
                              <div className={`flex items-center gap-1 px-2 py-1 rounded ${
                                review.rating >= 4.5 ? 'bg-green-50' :
                                review.rating >= 3.5 ? 'bg-yellow-50' :
                                'bg-red-50'
                              }`}>
                                ...
                                <span className={`text-sm font-medium ${
                                  review.rating >= 4.5 ? 'text-green-700' :
                                  review.rating >= 3.5 ? 'text-yellow-700' :
                                  'text-red-700'
                          ...
                          <div className="flex items-center gap-3 text-sm text-gray-500">
                            <span className="flex items-center gap-1">
                              ...
                              {review.submittedAt ? new Date(review.submittedAt).toLocaleDateString() : 'Date N/A'}
                            </span>
                            {review.channel && (
