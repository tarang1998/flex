...
export async function GET(
  ...
  try {
    ...
    const [hostawayReviews, mockReviews, googleReviews, approvedReviewIds] = await Promise.all([
      ...
      getMockReviews.execute([listingId]),
      getReviewsFromGoogle.execute(),
      getApprovedReviewIdsByListing.execute(listingId),
    ]);
    ...
    const averageReviewRating = calculateAverageRating(allReviews);
    ...
    console.error('Error in /api/dashboard/listing/[id]:', error);
...
function generateActionItems(
  ...
  approvedAverageRating: number,
  categoryAverages: Record<string, number>
): Array<{ priority: 'critical' | 'high' | 'medium' | 'low'; issue: string; action: string; impact: string }> {
  const actionItems: Array<{ priority: 'critical' | 'high' | 'medium' | 'low'; issue: string; action: string; impact: string }> = [];
